...
class AttentionModel_2(tf.keras.Model):
    """ Attention model """
    ...
    def __init__(self, action_space):
        ...
        self.source1 = tf.keras.layers.Lambda(get_source_layer1, output_shape=(-1, 3136))
        self.target1 = tf.keras.layers.Lambda(get_target_layer1, output_shape=(-1, 3136))
        ...
        self.source10 = tf.keras.layers.Lambda(get_source_layer10, output_shape=(-1, 3136))
        self.target10 = tf.keras.layers.Lambda(get_target_layer10, output_shape=(-1, 3136))
    ...
    def call(self, x):  # x:(3,5,84,84,4)
        ...
        source1 = self.source1(x0)  # (3,1,3136)
        target1 = self.target1(x0)  # (3,4,3136)

        x2, score1 = self.attention1(source1, target1,
                                     return_attention_scores=True)  # (3,1,3136), (3,4,1,4)

        x4 = self.flatten2(x2)  # (3,3136)

        x5 = self.concat1([x0[:, -1, :], x4])  # (3,6272)
        ...
        source10 = self.source10(x00)  # (3.1,3136)
        target10 = self.target10(x00)  # (3,4,3136)

        x20, score10 = self.attention10(source10, target10,
                                        return_attention_scores=True)  # (3,5,3136), (3,4,1,4)

        x40 = self.flatten20(x20)  # (3,3136)

        x50 = self.concat10([x00[:, -1, :], x40])  # (3,6272)
...
if __name__ == '__main__':
    ...
    policy = AttentionModel_2(action_space=2)
